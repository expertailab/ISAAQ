...
def get_data_ndq(dataset_name, split, tokenizer, max_len):
    input_ids_list=[]
    att_mask_list=[]
    labels_list=[]
    cont = 0
    with open("../jsons/"+dataset_name+".json", "r", encoding="utf-8", errors="surrogatepass") as file:
...
def get_data_VQA(split, tokenizer, max_len):
    input_ids_list=[]
    att_mask_list=[]
    images_list=[]
    labels_list=[]
    cont = 0
    with open("../jsons/vqa.json", "r", encoding="utf-8", errors="surrogatepass") as file:
        dataset = json.load(file)
    dataset = [doc for doc in dataset if doc["split"] == split]
    for doc in tqdm(dataset):
        ...
        att_mask_q =[]
        images_q=[]
        for count_i in range(18):
            images_q.append("VQA/"+doc["image_path"])
            try:
                answer = answers[count_i]
            except:
                answer = ""
        ...
        label = list(doc["answers"].keys()).index(doc["correct_answer"])
        labels_list.append(label)
    return [input_ids_list, att_mask_list, images_list, labels_list]
...
def get_data_pretrainings(dataset_name, split, tokenizer, max_len):
    input_ids_list=[]
    att_mask_list=[]
    labels_list=[]
    cont = 0
    with open("../jsons/"+dataset_name+".json", "r", encoding="utf-8", errors="surrogatepass") as file:
        dataset = json.load(file)
    dataset = [doc for doc in dataset if doc["split"] == split and len(doc["choices"])==4]
    for doc in tqdm(dataset):
        question = doc["question"]
        choices = doc["choices"]
        input_ids_q = []
        ...
        for choice in choices:
            ...
            labels_aux.append(choice["label"])
            input_ids_aux, att_mask_aux = get_choice_encoded(text, question, answer, max_len, tokenizer)
            input_ids_q.append(input_ids_aux)
            att_mask_q.append(att_mask_aux)
        input_ids_list.append(input_ids_q)
        att_mask_list.append(att_mask_q)
        label = labels_aux.index(doc["correct_answer"])
        labels_list.append(label)
    return [input_ids_list, att_mask_list, labels_list]

def get_data_AI2D(split, tokenizer, max_len):
    input_ids_list=[]
    att_mask_list=[]
    images_list = []
    coords_list = []
    labels_list=[]
    cont = 0
    with open("../jsons/ai2d.json", "r", encoding="utf-8", errors="surrogatepass") as file:
        dataset = json.load(file)
    dataset = [doc for doc in dataset if doc["split"] == split]
    for doc in tqdm(dataset):
        question = doc["question"]
        ...
        for count_i in range(4):
            ...
            input_ids_q.append(input_ids_aux)
            att_mask_q.append(att_mask_aux)
            images_q.append("AI2D/"+ doc["image_path"])
...
def training_ndq(model, train_dataloader, val_dataloader, optimizer, scheduler, epochs, device, save_model, dataset_name):
    for epoch_i in range(0, epochs):
        ...
        if save_model:
            torch.save(model, "../checkpoints/"+dataset_name+"_e"+str(epoch_i+1)+".pth")
...
def training_AI2D(model, raw_data_train, raw_data_val, optimizer, scheduler, epochs, batch_size, device, save_model):
    for epoch_i in range(0, epochs):
        ...
        if save_model:
            torch.save(model, "../checkpoints/AI2D_e"+str(epoch_i+1)+".pth")
        
        validation_AI2D(model, raw_data_val, batch_size, device)
        
    print("")
    print("Training complete!")
    
def validation_AI2D(model, raw_data_val, batch_size, device):
...
def training_VQA(model, raw_data_train, raw_data_val, optimizer, scheduler, epochs, batch_size, device, save_model):
    for epoch_i in range(0, epochs):
        ...
        input_ids_list, att_mask_list, images_list, labels_list = raw_data_train
        ...
        if save_model:
            torch.save(model, "../checkpoints/VQA_e"+str(epoch_i+1)+".pth")
        
        validation_VQA(model, raw_data_val, batch_size, device)
        
    print("")
    print("Training complete!")
    
def validation_VQA(model, raw_data_val, batch_size, device):
    ...
    input_ids_list, att_mask_list, images_list, labels_list = raw_data_val
